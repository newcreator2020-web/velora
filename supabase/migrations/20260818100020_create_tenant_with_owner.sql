-- ====================================================================
-- FASE 2 — Onboarding: RPC atomica creazione tenant + owner + business profile
--
-- 1. NUOVA MIGRAZIONE APPEND-ONLY: NON modifica migration FASE 1 congelate.
-- 2. SECURITY DEFINER: helper di provisioning, server-side auth.uid() UNICA fonte.
-- 3. SET search_path = '' + oggetti public.* fully qualified → no path hijack.
-- 4. Input validati e sanitizzati: slug normalizzato, stringhe vuote → NULL.
-- 5. Transazione atomica: qualsiasi errore ROLLBACK totale.
-- 6. Idempotenza double-submit: se owner già esiste attivo → return esistente.
-- 7. UNIQUE(tenants.slug) = autorità finale sulle collisioni (race condition).
-- 8. Last owner invariant: nasce SEMPRE almeno 1 owner (la membership) → rispettato.
-- 9. Audit 2 eventi: tenant_created + onboarding_completed.
-- 10. Privilegi: REVOKE PUBLIC + solo EXECUTE a ruolo authenticated.
-- ====================================================================
...
BEGIN
  ...
  v_base := regexp_replace(
              lower(unaccent(v_business_name)),
              '[^a-z0-9]+', '-', 'g');
  ...
  -- ------------------------------------------------------------------
  -- 5. Membership OWNER — invariante last owner = creata contestualmente
  --    al tenant; trigger guard_last_active_owner valida poi in DELETE/UPDATE.
  -- ------------------------------------------------------------------
  ...
  INSERT INTO public.business_profiles(
    id, tenant_id, display_name, description, category, city, province,
    address_line1, phone, website_url, email, timezone, locale
  )
  VALUES (
    v_bp_id, v_tenant_id, v_business_name, '', v_category, v_city, v_province,
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Audit failure non deve rollback provisioning, ma non deve neppure
    -- rompere l'onboarding. Ignoriamo silenziosamente qui; il monitoraggio
    -- prod può triggerare alert sul missing audit tramite queue separata.
    NULL;
  END;
...
EXCEPTION WHEN OTHERS THEN
  -- Qualsiasi eccezione → ROLLBACK (inclusa unique slug, fk, check).
  -- Nessun orfano. Errore user friendly senza leaking stack.
  RAISE;
END;
$$;

-- --------------------------------------------------------------------
-- 9. Privilegi: autenticati = unici a poter eseguire; PUBLIC revocato.
-- --------------------------------------------------------------------
