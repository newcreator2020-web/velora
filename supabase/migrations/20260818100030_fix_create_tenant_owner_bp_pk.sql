-- ====================================================================
-- FASE 2 — FIX RPC create_tenant_with_owner
--
-- Motivo:
--   public.business_profiles ha PRIMARY KEY (tenant_id), NON (id).
--   La RPC precedente usava bp.id e INSERT con column id → errori.
--
-- Modifiche:
--   * query v_existing → bp.tenant_id AS business_profile_id
--   * INSERT business_profiles → REMOVED column id, PK = tenant_id
--
-- Append-only. Nessuna alterazione a migration 20260818100020 congelata.
-- ====================================================================
...
BEGIN
  ...
  v_base := regexp_replace(lower(unaccent(v_business_name)), '[^a-z0-9]+', '-', 'g');
