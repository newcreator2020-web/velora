-- FASE14C - HOTFIX #76 - FINAL.
-- CRITICAL: la frozen schema public.tenants NON contiene legal_business_name/category/city/
-- province/phone/business_email/timezone/locale/created_by_user_id. Queste colonne sono
-- presenti solo su public.business_profiles (1:1 con tenant).
-- Trigger create_business_profile_after_tenant crea il BP vuoto dopo INSERT tenants.
-- Corregiamo la RPC platform_provision_customer inserendo SOLO colonne realmente presenti
-- in tenants, e popoliamo business_profiles tramite UPDATE successivo (già creato da trigger).
...
BEGIN
    ...
    -- IDEMPOTENCY: se ultima richiesta (10 min) stesso slug/owner/plan e andata a buon fine -> CONFLICT
    IF EXISTS(
    ...
    -- ATOMIC PROVISIONING
    -- ============================================================
    -- 1. INSERT tenants (solo colonne realmente presenti nello schema frozen FASE7)
    INSERT INTO public.tenants (
        ...
        v_tenant_id,
        TRIM(p_business_name),          -- name
        v_slug,
        'active',                       -- status -> 'active' (invece di onboarding)
        p_plan_id,
        v_temp_domain,
        false,                          -- published
        v_now,
    ...
    -- 2. OWNER membership — esattamente una volta
    IF NOT EXISTS(
    ...
    -- Il trigger create_business_profile_after_tenant (frozen) ha creato un BP vuoto
    -- con PK tenant_id=v_tenant_id. Popoliamo campi noti.
...
EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
        -- Audit failure non-blocking (best effort)
        BEGIN
            INSERT INTO public.audit_logs (
                id, tenant_id, actor_user_id, action, target_type, target_id,
                metadata, created_at
