-- FASE14C - HOTFIX #78.
-- Problema: il frozen trigger create_business_profile_after_tenant NON crea sempre la riga bp.
-- Oppure RLS/permessi ne impediscono la creazione nel contesto SECURITY DEFINER search_path=''.
-- Soluzione: invece di UPDATE (che non produce nulla se manca la riga), usiamo INSERT ... ON CONFLICT.
-- Richiede PK public.business_profiles(tenant_id) — già verificata probe (tenant_id è colonna PK-like).
...
BEGIN
    ...
    -- 3. Business profile (INSERT garantito, non solo UPDATE. Se trigger ha creato → conflict → update)
    INSERT INTO public.business_profiles (
        tenant_id, display_name, legal_name, category, phone, email,
        city, province, timezone, locale, created_at, updated_at
    ) VALUES (
        v_tenant_id,
        TRIM(p_business_name),
        TRIM(p_business_name),
        TRIM(COALESCE(p_category, 'service_business')),
        NULLIF(TRIM(p_phone), ''),
        NULLIF(TRIM(p_business_email), ''),
        TRIM(COALESCE(p_city, 'Non specificata')),
        TRIM(COALESCE(p_province, '--')),
        TRIM(COALESCE(p_timezone, 'Europe/Rome')),
        TRIM(COALESCE(p_locale, 'it-IT')),
        v_now, v_now
    )
